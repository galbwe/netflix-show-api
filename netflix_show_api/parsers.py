...
from typing import Any, Callable, FrozenSet, List, NamedTuple, Optional, Tuple


def parse_delimited(fields: Optional[str], delim=",") -> Optional[FrozenSet[str]]:
    if fields is None:
        return
    return frozenset(field.strip() for field in fields.split(delim))
...
def parse_order_by(
    fields: Optional[str], delim=",", descending_postfix=_DESCENDING_POSTFIX
) -> Tuple[OrderByParam]:
    if fields is None:
        return
    params = []
...
FilterOperator = Enum(
    "FilterOperator",
    ["EQUAL", "LIKE", "GREATER_THAN", "LESS_THAN", "GREATER_THEN_OR_EQUAL", "LESS_THAN_OR_EQUAL"],
)
